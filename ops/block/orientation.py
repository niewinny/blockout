...
def face_orientation(cls, context):
    ...
    match align_face:
        ...
        case 'EDGE': _edge, direction_local, _normal_local = orientation.direction_from_closest_edge(hit_obj, hit_face, loc)
...
def edge_orientation(cls, context):
    ...
    edge, direction_local, normal_local = orientation.direction_from_closest_edge(hit_obj, hit_face, loc_world)
    direction_world = cls.ray.obj.matrix_world.to_3x3() @ direction_local
    ...
    normal_projected = world_normal - world_normal.dot(direction_world) * direction_world
    ...
    dot_product = max(min(dot_product, 1.0), -1.0)  # Clamp to avoid floating point errors
