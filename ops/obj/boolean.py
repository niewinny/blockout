...
class BooleanOperatorBase(bpy.types.Operator):
    ...
    solver: bpy.props.EnumProperty(name="Solver", items=get_solver_items, default=0)
    ...
    def execute(self, context):
        ...
        if self.flip:
            self.apply_operation_active_to_selected(
                context, selected_objects, active_object
            )
        else:
            self.apply_operation_selected_to_active(
                context, selected_objects, active_object
            )

        # Handle post-operation selection if defined
        if hasattr(self, "update_selection_after_operation"):
            self.update_selection_after_operation(
    ...
    def validate_selection(self, selected_objects):
        ...
        if len(selected_objects) < 1:
            self.report(
                {"WARNING"}, "Not enough objects selected for boolean operation"
            )
            return {"CANCELLED"}
        return None

    def apply_operation_active_to_selected(
        self, context, selected_objects, active_object
    ):
        """Override this method in subclasses to implement active->selected operation"""
        raise NotImplementedError(
            "Subclasses must implement apply_operation_active_to_selected"
        )

    def apply_operation_selected_to_active(
        self, context, selected_objects, active_object
    ):
        """Override this method in subclasses to implement selected->active operation"""
        raise NotImplementedError(
            "Subclasses must implement apply_operation_selected_to_active"
        )
...
class BOUT_OT_ModBoolean(BooleanOperatorBase):
    ...
    def update_selection_after_operation(
        self, context, active_object, selected_objects
    ):
    ...
    def _create_carve_duplicate(self, context, obj):
        ...
        if self.offset > 0:
            ...
            for vert in bm.verts:
                ...
                vert_normal = sum(
                    (f.normal for f in vert.link_faces), start=Vector((0, 0, 0))
                )
