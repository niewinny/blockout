...
def setup_drawing(context, ui, points=None):
    ...
    ui.guide.handle = bpy.types.SpaceView3D.draw_handler_add(ui.guide.callback.draw, (context,), 'WINDOW', 'POST_VIEW')
    
    lines = []
    ui.interface.create(context, lines=lines)


def update_drawing(context, ui, mouse_median, mouse_co, width, segments, limit_method, angle_limit, modifier_count_text):
    '''Update the drawing'''
    _theme = addon.pref().theme.ops.obj.bevel
    color = _theme.guide
...
def set_segments_from_mouse(context, event, mouse_median, saved_mouse_pos, saved_segments):
