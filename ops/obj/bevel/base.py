...
from .data import Mouse, Distance, DrawUI, Bevel
from . import utils
...
class BevelOperatorBase(bpy.types.Operator):
    ...
    def modal(self, context, event):
        if event.type == 'MOUSEMOVE':
            intersect_point = utils.get_intersect_point(context, event, self.mouse.median)
    ...
    def _calculate_distance(self):
        ...
        return utils.calculate_distance(self.mouse.median, self.mouse.init, self.mouse.co)
    ...
    def _update_drawing(self, context):
        ...
        utils.update_drawing(
            context, self.ui, self.mouse.median, self.mouse.co,
            self.width, self.segments, self.limit_method, self.angle_limit,
