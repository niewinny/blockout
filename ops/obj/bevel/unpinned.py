...
class BevelModifierItem(bpy.types.PropertyGroup):
    ...
    was_created: bpy.props.BoolProperty(name="Was Created", description="Whether this modifier was created by the operator")
...
def update_selected_modifier(self, context):
    ...
    if hasattr(self, '_previous_index') and 0 <= self._previous_index < len(self.edited_modifiers):
...
class BOUT_OT_ModBevel(BevelOperatorBase):
            ...
            for obj in selected_objects:
                for mod in obj.modifiers:
                    if mod.type == 'BEVEL' and not mod.use_pin_to_last:
                        ...
                        item.was_created = (obj.name, mod.name) not in existing_modifiers
    ...
    def _store_edited_modifiers(self):
        ...
        for bevel in self.bevels:
            ...
            item.was_created = bevel.new  # Track if this modifier was created by the operator
    ...
    def draw(self, context):
        ...
        if self.edited_modifiers:
            ...
            row = box.row()
            row.template_list(
                "UI_UL_list", "bevel_modifiers_list",
                self, "edited_modifiers",
                self, "selected_modifier_index",
                rows=3, maxrows=6
            )
