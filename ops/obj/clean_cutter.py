...
class BOUT_OT_CleanCutter(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        if objects_in_use:
            ...
            return context.window_manager.invoke_props_dialog(
                self, width=400, confirm_text="Duplicate"
            )
    ...
    def draw(self, context):
        ...
        if hasattr(self, "objects_in_use"):
            ...
            col.label(
                text="Warning: Selected objects are used by modifiers!", icon="ERROR"
            )
            col.separator()
            ...
            if len(self.objects_in_use) > 5:
                box.label(
                    text=f"  ... and {len(self.objects_in_use) - 5} more", icon="DOT"
                )
    ...
    def execute(self, context):
        ...
        if duplicated_count > 0:
            self.report(
                {"INFO"},
                f"Cleaned {cleaned_count} object(s) ({duplicated_count} duplicated)",
            )
        else:
